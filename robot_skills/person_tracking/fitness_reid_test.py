...
class PersonDetector:
    ...
    def __init__(self, model: str, weight: str, confidence: float, core_mask: int):
        ...
        ret = self.rknn.init_runtime(target="rk3588", core_mask=int(core_mask), device_id=ids[0])
        if ret != 0:
...
class RknnReID:
    ...
    def feature(self, crop: np.ndarray) -> np.ndarray:
        ...
        data = np.expand_dims(np.transpose(rgb, (2, 0, 1)), 0)
...
def build_models(config: dict[str, Any]) -> tuple[PersonDetector, RknnReID]:
    ...
    detector = PersonDetector(models["person_detector"], models["person_detector_weight"], detector_cfg["confidence"], detector_cfg["npu_core_mask"])
    try:
