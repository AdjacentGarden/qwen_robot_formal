...
parser=argparse.ArgumentParser(description='Turn left in place skill.')
...
node=rclpy.create_node('skill_move_left')
...
try:
    ...
    if require_subscriber and subscribers<=0:
        print(json.dumps({'ok': False, 'skill': 'move_left', 'topic': args.topic, 'duration': args.duration, 'subscribers': subscribers, 'error': 'cmd_vel_subscribers_0'}, ensure_ascii=False))
        raise SystemExit(3)
    ...
    print(json.dumps({'ok': True, 'skill': 'move_left', 'topic': args.topic, 'duration': args.duration, 'subscribers': subscribers}, ensure_ascii=False))
finally:
