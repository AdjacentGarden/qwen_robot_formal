...
# ===== Speaker bridge =====
_speaker_bridge = types.ModuleType('speaker')
_speaker_bridge._mp_q = None
_speaker_bridge._local_engine = None

class _FixedDoubaoAudioSpeaker:
    def __init__(self):
        self.audio_dir = Path(os.getenv('DOUBAO_FIXED_AUDIO_DIR', '/home/test/refine_0508/llm/doubao_fixed_audio'))
        self.output_device = os.getenv(
            'DOUBAO_REALTIME_OUTPUT_DEVICE',
            os.getenv('SINGLE_FUNCTION_TTS_DEVICE', 'plughw:rockchiptas6424'),
        )
    ...
    def _load_manifest(self, force=False):
        manifest = self.audio_dir / 'manifest.json'
        try:
        ...
        try:
            ...
            self.channels = int(data.get('channels', self.channels))
            mapping = {}
            for item in data.get('items', []):
                text = str(item.get('text', '')).strip()
                rel_file = str(item.get('file', '')).strip()
                if not text or not rel_file:
                    continue
                path = self.audio_dir / rel_file
                if path.exists():
                    mapping[text] = path
            self._text_to_file = mapping
            self._manifest_mtime = mtime
        except Exception as exc:
            print(f'[Speaker Warning] failed to load Doubao fixed audio manifest: {exc}')
            if force:
                self._text_to_file = {}
    ...
    def find_audio(self, text):
        ...
        if not text:
            return None
        self._load_manifest()
        return self._text_to_file.get(text)

    def speak(self, text):
        text = str(text or '').strip()
        if not text:
            return True
        path = self.find_audio(text)
        if path is None:
            print(f'[Speaker Warning] no cached Doubao audio for: {text}')
            return False
        try:
            import subprocess
            ...
            if completed.returncode != 0:
                print(f'[Speaker Warning] aplay failed: {completed.stderr.strip()}')
                return False
            return True
        except Exception as exc:
            print(f'[Speaker Warning] fixed Doubao audio playback failed: {exc}')
            return False

def _bridge_init_mp_queue(q=None):
    _speaker_bridge._mp_q = q
    return True

def _bridge_speak(text):
    text = str(text or '').strip()
    if not text:
        return True
    if _speaker_bridge._mp_q is not None:
        try:
            _speaker_bridge._mp_q.put(text)
            return True
        except Exception as exc:
            print(f'[Speaker Warning] failed to enqueue TTS text: {exc}')
    if os.getenv('SINGLE_FUNCTION_LOCAL_TTS', '1').strip().lower() in {'0', 'false', 'no', 'off'}:
        return False
    if _speaker_bridge._local_engine is None:
        _speaker_bridge._local_engine = _FixedDoubaoAudioSpeaker()
    return _speaker_bridge._local_engine.speak(text)

_speaker_bridge.init_mp_queue = _bridge_init_mp_queue
_speaker_bridge.speak = _bridge_speak
_speaker_bridge.stop = lambda: True
_speaker_bridge.release = lambda: True
sys.modules['speaker'] = _speaker_bridge
speaker = _speaker_bridge
...
# ===== Inlined module: movement_count_2.pullup_camera_base_fps_1 =====
_before_inline = set(globals())
_run_file = __file__
__file__ = str(SKILL_DIR / 'embedded_sources' / 'movement_count_2/pullup_camera_base_fps_1.py')
import os
import sys

# 当前文件所在目录：
# /home/test/refine_0508/llm/movement_count_2
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# 为了能导入:
# from skills import runtime_config, runtime_models
# 需要加入 /home/test/refine_0508
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)

# 为了能导入:
# import poseembedding
# import poseclassifier
# import resultsmooth
# import counter
# 需要加入 /home/test/refine_0508/movement_count_2
CODE_DIR = SCRIPT_DIR
...
from movement_count_2 import visualizer as vs
...
class PullupCountingSystem:
    ...
    对外主调用方法：
        1. start_pullup_counting(video_source)
           开始引体向上计数。

        2. query_pullup_count()
           查询当前引体向上个数。

        3. stop_pullup_counting()
           结束引体向上计数并汇总。
    ...
    Board 生命周期原则：
        ...
        2. 当前引体向上计数默认不需要轮子、步进电机和天问模块，所以默认不创建 Board。
        3. 如果未来某个计数功能需要用到底轮、步进电机或天问模块，
           只在该功能开始时初始化 Board，功能结束后在 finally 中释放 Board。
    """

    DEFAULT_PID_FILE = "/tmp/pullup_pid.txt"
    DEFAULT_COUNT_FILE = "/tmp/pullup_count.txt"
    ...
    def _create_board():
        ...
        当前引体向上计数默认不需要 Board，所以默认不会调用该函数。
        后续如果需要例如：
            - 计数时自动调整机器人位置；
            - 使用步进电机调整摄像头角度；
            - 使用天问模块做唤醒检测；
        ...
        try:
            ...
            board = Board()
            print("[PullupCountingSystem] Board 已初始化，仅当前功能使用")
            return board

        except ImportError:
            print("[PullupCountingSystem] 底盘驱动 test3.Board 导入失败，使用虚拟 Board")
            return PullupCountingSystem.DummyBoard()

        except Exception as e:
            print(f"[PullupCountingSystem] Board 初始化失败，使用虚拟 Board: {e}")
            return PullupCountingSystem.DummyBoard()
    ...
    def _release_board(board):
        ...
        for method_name in ("release", "close", "shutdown", "stop"):
            try:
                ...
                if callable(method):
                    method()
                    print(f"[PullupCountingSystem] Board.{method_name}() 已调用")
                    return
            except Exception as e:
                print(f"[PullupCountingSystem] 调用 Board.{method_name}() 失败: {e}")

        try:
            port = getattr(board, "port", None)
            if port is not None and hasattr(port, "close"):
                port.close()
                print("[PullupCountingSystem] Board 串口 port 已关闭")
        except Exception as e:
            print(f"[PullupCountingSystem] 关闭 Board 串口失败: {e}")
    ...
    def _start_board_reception_if_needed(board):
        ...
        try:
            if hasattr(board, "enable_reception"):
                ...
                print("[PullupCountingSystem] Board 接收线程已启动")
        except Exception as e:
            print(f"[PullupCountingSystem] Board 接收线程启动失败: {e}")
    ...
    def background_counting_task(
        ...
        try:
            if need_board:
                board = PullupCountingSystem._create_board()
                board_owned = not isinstance(board, PullupCountingSystem.DummyBoard)

                # 如果后续需要天问模块，取消下面这一行的注释。
                # PullupCountingSystem._start_board_reception_if_needed(board)

            det, det_owned = runtime_models.acquire_or_create(
                "pullup_detector",
                PullupCountingSystem.PullupDet,
                description="引体向上 MediaPipe 姿态检测器",
            )

            cap = PullupCountingSystem.CameraReader(video_source)
            ...
            single_subject_gate = PullupCountingSystem.SingleSubjectGate()

            initial_count = max(0, int(initial_count or 0))
            initial_elapsed_seconds = max(0.0, float(initial_elapsed_seconds or 0.0))
            PullupCountingSystem._write_count_file(count_file_path, initial_count)

            if start_gate_path:
                _single_function_emit_ready("pull_up", "请准备好，1，2，3，开始")
            ...
            if resume_from_interrupt and initial_count > 0:
                PullupCountingSystem._safe_speak(
                    f"继续引体向上计数，之前已经完成{number_to_chinese(initial_count)}个。",
            ...
            while cap.isOpened() and is_running:
                ...
                if now - session_start >= PullupCountingSystem.SESSION_SECONDS:
                    PullupCountingSystem._safe_speak(
                        f"三十秒已到，本次引体向上计数结束，共做了{number_to_chinese(last_count)}个。",
                        speaker_module=speaker,
                    )
                    break

                if now - last_motion_time >= PullupCountingSystem.IDLE_SECONDS:
                    PullupCountingSystem._safe_speak(
                        f"连续十秒没有检测到新动作，本次引体向上计数自动结束，共做了{number_to_chinese(last_count)}个。",
                        speaker_module=speaker,
                    )
                    break
                ...
                current_total_count = initial_count + pullup_count

                if pullup_count > last_session_count:
                    ...
                    last_session_count = pullup_count
                    ...
                    PullupCountingSystem._write_count_file(count_file_path, last_count)

                    print(f"[计数更新] Pull-ups: {last_count}")
            ...
            if board_owned:
                PullupCountingSystem._release_board(board)
                board = None
    ...
    def start_pullup_counting(self, video_source, need_board: bool = False, tts_mp_q=None, start_gate_path=None, initial_count: int = 0, resume_from_interrupt: bool = False, initial_elapsed_seconds: float = 0.0):
        """
        开始引体向上计数。
        ...
        print(
            f"引体向上计数已启动，PID={p.pid}，"
            f"将自动运行 {self.SESSION_SECONDS} 秒；"
            f"连续 {self.IDLE_SECONDS} 秒无动作会提前结束。"
        )

        return True

    def query_pullup_count(self) -> int:
        """
        查询当前引体向上个数。
    ...
    def stop_pullup_counting(self) -> int:
        """
        结束引体向上计数并汇总。
    ...
    def start_counting(self, video_source, tts_mp_q=None, start_gate_path=None, initial_count: int = 0, resume_from_interrupt: bool = False, initial_elapsed_seconds: float = 0.0):
        """
        兼容旧代码：
            pullup_sys.start_counting(video_source)
        """
        return self.start_pullup_counting(video_source, need_board=False, tts_mp_q=tts_mp_q, start_gate_path=start_gate_path, initial_count=initial_count, resume_from_interrupt=resume_from_interrupt, initial_elapsed_seconds=initial_elapsed_seconds)
    ...
    def query_progress(self):
        ...
        """
        return self.query_pullup_count()
    ...
    def stop_and_summarize(self):
        ...
        """
        return self.stop_pullup_counting()
...
if False and __name__ == "__main__":
    ...
    while True:
        ...
        if cmd == "e":
            pullup_sys.stop_pullup_counting()
            break

        elif cmd == "s":
            pullup_sys.start_pullup_counting(CAMERA_ID)

        elif cmd == "q":
            pullup_sys.query_pullup_count()
            ...
            print("无效指令")
__file__ = _run_file
_publish_namespace('movement_count_2.pullup_camera_base_fps_1', _before_inline, top_level_aliases=())
del _before_inline
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Pull-up' + ' counting skill.')
    ...
    PullupCountingSystem.SESSION_SECONDS = max(1, int(args.duration))

    def wait_for_process_or_timeout(max_seconds):
        process = getattr(system, "_process", None)
        if process is None:
            time.sleep(max(1, int(max_seconds)))
            return
    ...
    if args.action == 'start':
        ...
        system.start_counting(args.camera or DEFAULT_CAMERA, start_gate_path=args.start_gate, initial_count=args.initial_count, resume_from_interrupt=args.resume_from_interrupt, initial_elapsed_seconds=args.initial_elapsed_seconds)
        wait_for_process_or_timeout(args.duration)
        print(system.stop_and_summarize())
