...
parser=argparse.ArgumentParser(description='Turn right in place skill.')
...
node=rclpy.create_node('skill_move_right')
...
try:
    ...
    if require_subscriber and subscribers<=0:
        print(json.dumps({'ok': False, 'skill': 'move_right', 'topic': args.topic, 'duration': args.duration, 'subscribers': subscribers, 'error': 'cmd_vel_subscribers_0'}, ensure_ascii=False))
        raise SystemExit(3)
    end=time.time()+max(0.05,float(args.duration))
    while time.time()<end:
        publish(0.0, -abs(args.angular_speed)); rclpy.spin_once(node, timeout_sec=0.02); time.sleep(0.08)
    for _ in range(6):
    ...
    print(json.dumps({'ok': True, 'skill': 'move_right', 'topic': args.topic, 'duration': args.duration, 'subscribers': subscribers}, ensure_ascii=False))
finally:
