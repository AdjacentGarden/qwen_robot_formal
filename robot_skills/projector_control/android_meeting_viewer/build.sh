...
BUILD="$ROOT/build"
OUT="$SKILL_ROOT/assets/meeting-image-viewer.apk"
...
test -s "$SKILL_ROOT/assets/test-1.jpg"
test -s "$SKILL_ROOT/assets/test-2.jpg"

rm -rf "$BUILD"
mkdir -p "$BUILD/gen" "$BUILD/classes" "$BUILD/dex" "$BUILD/res/drawable-nodpi"
cp -R "$ROOT/res/." "$BUILD/res/"
cp "$SKILL_ROOT/assets/test-1.jpg" "$BUILD/res/drawable-nodpi/meeting_slide_1.jpg"
cp "$SKILL_ROOT/assets/test-2.jpg" "$BUILD/res/drawable-nodpi/meeting_slide_2.jpg"

"$TOOLS/aapt" package -f -m -J "$BUILD/gen" -M "$ROOT/AndroidManifest.xml" \
  -S "$BUILD/res" -I "$ANDROID_JAR"
javac -source 8 -target 8 -Xlint:-options -bootclasspath "$ANDROID_JAR" \
  -d "$BUILD/classes" \
  "$BUILD/gen/com/adjacentgarden/meeting/R.java" \
  "$ROOT/src/com/adjacentgarden/meeting/MeetingSlidesActivity.java"
"$TOOLS/d8" --lib "$ANDROID_JAR" --output "$BUILD/dex" \
  $(find "$BUILD/classes" -name '*.class' -print)
"$TOOLS/aapt" package -f -M "$ROOT/AndroidManifest.xml" -S "$BUILD/res" \
