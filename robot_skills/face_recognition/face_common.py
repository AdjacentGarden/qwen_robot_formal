...
# Optional objects injected by the resident runtime.  Standalone execution sees
# the default None values and preserves the original allocation/release path.
_RESIDENT_MODEL_OVERRIDE = None
_RESIDENT_DETECTOR_OVERRIDE = None
_RESIDENT_CAMERA_PROVIDER = None
...
def _open_camera(source, cfg):
    ...
    if callable(_RESIDENT_CAMERA_PROVIDER):
        cap, debug = _RESIDENT_CAMERA_PROVIDER(resolved, cfg)
        payload = dict(debug or {})
        payload.setdefault("requested", requested)
        payload.setdefault("resolved", str(resolved))
        payload.setdefault("attempts", [{"attempt": 1, "opened": bool(cap.isOpened()), "read_ok": True}])
        payload.setdefault("open_retry_sec", 0.0)
        return cap, payload
    attempts = []
...
def register_face(cfg, name, camera=None, start_gate_path=None):
    ...
    model = detector = None
    model_owned = detector_owned = False
    last_snapshot = None
    collected_aligned = []
    collected_legacy = []
    ...
    last_extract_ts = 0.0
    try:
        model = _RESIDENT_MODEL_OVERRIDE or RKNNFaceEmbeddingModel(cfg.model_path)
        detector = _RESIDENT_DETECTOR_OVERRIDE or RetinaFaceRKNN(cfg.detector_model_path)
        model_owned = _RESIDENT_MODEL_OVERRIDE is None
        detector_owned = _RESIDENT_DETECTOR_OVERRIDE is None
        if start_gate_path:
        ...
        cap.release()
        if model is not None and model_owned:
            model.release()
...
def run_registration_cli(skill_dir, argv_override=None):
    started = time.time()
    argv = _preflight("face_registration", list(__import__("sys").argv[1:] if argv_override is None else argv_override))
...
def run_recognition_cli(skill_dir, argv_override=None):
    started = time.time()
    argv = _preflight("face_recognition", list(__import__("sys").argv[1:] if argv_override is None else argv_override))
