...
class RosRobotController(Node):
    ...
    def _head_control_loop(self):
        ...
        while self.head_running.is_set() and not self.is_shutting_down:
            ...
            # A timed-out command must stop producing motor packets.  Keeping
            # the feedback loop active after its deadline caused the real head
            # to oscillate around level indefinitely while the caller already
            # held an unknown result.  The target is retained for diagnostics;
            # a later explicit command resets the timeout latch.
            if settled or timed_out:
                motor_cmd = 0.0
