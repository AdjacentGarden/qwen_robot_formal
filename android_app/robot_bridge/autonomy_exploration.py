...
class AutonomyEngine:
    ...
    def _record_found_pet_video(self, session: Path, duration_sec: float = 5.0) -> dict[str, Any]:
        """Record a steady post-detection clip without reopening the physical camera.

        The resident camera manager reads the already-running shared-memory camera
        broker.  A paced loop is important here: broker reads are non-blocking, so
        writing as fast as possible would create a clip with an incorrect duration.
        """
        fps = 15.0
        raw_path = session / "doudou_found_raw.mp4"
        final_path = session / "doudou_found_5s.mp4"
        cap = self.owner.camera.lease("/dev/video22", 640, 480, fps)
        ok, frame = cap.read()
        ...
        try:
            while time.monotonic() - started < duration_sec:
                if self.owner.exploration_stop.is_set():
                    raise RuntimeError("pet_search_cancelled_during_video")
                ok, frame = cap.read()
            ...
            writer.release()
        if written < int(duration_sec * fps * 0.70):
        ...
        if conversion.returncode != 0 or not final_path.exists():
        ...
        try:
            ...
            for scan_index, region in enumerate(regions):
                ...
                if scan.get("found"):
                    ...
                    video_warning = None
                    try:
                        video = self._record_found_pet_video(session, duration_sec=5.0)
                    except Exception as exc:
                        # Finding the pet remains a successful outcome even if the
                        # optional phone-video delivery has a recoverable failure.
                        video_warning = f"{type(exc).__name__}: {exc}"
                    report = {
                        ...
                        "post_detection_video": video,
                        "session_dir": str(session), "elapsed_sec": round(time.monotonic() - started, 3),
                    }
                    if video_warning:
                        report["warnings"] = [f"post_detection_video_failed:{video_warning}"]
                    self._write_report(session / "report.json", report)
                    print(json.dumps(report, ensure_ascii=False))
                    return 0
                if not scan.get("ok"):
