...
class RobotOrchestrator:
    def __init__(self, config: dict[str, Any]):
        ...
        self.realtime_voice = DoubaoRealtimeSession(config, self.registry, self.resources) if config.get("audio", {}).get("voice_io_backend") == "doubao_realtime" else None
    ...
    def _handle_memory_turn(
        ...
        *,
        ...
        session.metadata.update(
            {
                "decision_backend": "doubao_realtime" if decision else "local_memory",
                "memory_operation": operation.get("operation"),
    ...
    def handle_voice_once(
        ...
        return {
            "ok": False,
            "error": "doubao_realtime_not_configured",
            "message": "voice command path requires audio.voice_io_backend=doubao_realtime",
        }
    ...
    def handle_voice_decision(
        ...
        if not decision.get("ok", True):
            ...
            session.ended_at = time.time()
            session.utterances.append({"role": "user", "text": "", "timestamp": time.time(), "decision_error": decision.get("error"), "decision_backend": "doubao_realtime"})
            self.store.save_session(session)
            self.store.append_event(
        ...
        if decision.get("decision_type") == "noop" and not decision.get("task_groups") and not decision.get("ask_user"):
            ...
            session.metadata["decision_backend"] = "doubao_realtime"
        ...
        if routed_followup is not None:
            return routed_followup
        session.utterances.append({"role": "user", "text": user_text, "timestamp": time.time(), "decision_backend": "doubao_realtime"})
        session.metadata["decision_backend"] = "doubao_realtime"
    ...
    def handle_followup_voice(self, task_group_id: str, seconds: float | None = None, execute: bool = False, dry_run: bool = True) -> dict[str, Any]:
        ...
        return {
            ...
            "task_group_id": task_group_id,
            "error": "doubao_realtime_not_configured",
            "message": "follow-up voice path requires audio.voice_io_backend=doubao_realtime",
        }
    ...
    def _prepare_followup_voice(self, task_groups: list[TaskGroup], ask_user: dict[str, Any]) -> dict[str, Any]:
        if self.realtime_voice is None or not isinstance(ask_user, dict):
            return {"ok": True, "skipped": True, "reason": "doubao_realtime_not_configured"}
