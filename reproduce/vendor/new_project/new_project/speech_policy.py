    ...
    def step_summary(self, step: Any, parsed: dict[str, Any]) -> str:
        ...
        if skill == "light_control":
            ...
            if action in {"status", "query"}:
                ...
                return "落地灯现在开着。" if power is True else "落地灯现在关着。" if power is False else "落地灯状态已经查询完成。"
            return "落地灯已经打开了。"
        ...
        if skill == "feeder_control":
            ...
            return f"已经投食{grams}克。" if grams is not None else "已经放好食物了。"
        ...
        if skill in {"person_tracking", "pet_tracking"}:
            target = "小狗" if str(arguments.get("pet") or "") == "dog" else "小猫" if str(arguments.get("pet") or "") == "cat" else "目标"
            if status in {"not_found", "no_target", "timeout"} or result.get("found") is False or parsed.get("found") is False:
                return f"这次没有找到{target}。"
            if status in {"cancelled", "interrupted"}:
                return f"已经停止跟随{target}。"
            return f"{target}的跟随已经结束，录像也保存好了。" if skill == "pet_tracking" else "人员跟随已经结束。"
        return ""
