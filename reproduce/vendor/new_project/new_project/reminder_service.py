...
def _reminder_timezone() -> ZoneInfo:
    name = os.getenv("REMINDER_TIMEZONE", "Asia/Shanghai").strip() or "Asia/Shanghai"
    try:
...
def _local_datetime(timestamp: float) -> datetime:
    return datetime.fromtimestamp(timestamp, tz=_reminder_timezone())
...
def parse_trigger(trigger_time: Any = None, trigger_condition: Any = None, now: float | None = None) -> tuple[float, str]:
    ...
    if relative:
        ...
        return due_at, f"{text}（{_local_datetime(due_at).strftime('%m月%d日 %H:%M:%S')}）"
    ...
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%m-%d %H:%M", "%H:%M:%S", "%H:%M"):
        with contextlib.suppress(ValueError):
            parsed = datetime.strptime(normalized, fmt).replace(tzinfo=_reminder_timezone())
            if fmt.startswith("%m"):
