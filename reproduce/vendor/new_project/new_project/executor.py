...
class SkillExecutor:
    ...
    def _pet_tracking_video_path(self) -> Path:
        return Path(self.config.get("paths", {}).get("single_function_dir", "/home/test/single_function")) / "pet_tracking" / "runtime" / "pet_tracking_record.mp4"
    ...
    def _augment_pet_find_result_from_result_file(self, result: dict[str, Any], started_at: float) -> dict[str, Any]:
        if isinstance(result.get("parsed_json"), dict):
            return result
        result_path = Path(self.config.get("paths", {}).get("single_function_dir", "/home/test/single_function")) / "pet_tracking" / "runtime" / "pet_tracking_result.txt"
        try:
    ...
    def _known_navigation_points(self) -> list[dict[str, Any]]:
        ...
        if not path:
            path = str(Path(self.config.get("paths", {}).get("single_function_dir", "/home/test/single_function")) / "points" / "named_points.json")
        try:
    ...
    def _single_function_result_from_completed(
        ...
        ok = completed.returncode == 0 and not interrupted
        parsed_error = ""
        if isinstance(parsed_json, dict) and parsed_json.get("error"):
            parsed_error = str(parsed_json.get("error"))
        error = "interrupted" if interrupted else ("" if completed.returncode == 0 else (parsed_error or (completed.stdout or "")[-1000:]))
        result = {
            "ok": ok,
            "interrupted": interrupted,
