...
class AudioManager:
    ...
    def speak_text(self, text: str) -> bool:
        ...
        try:
            if self.realtime_voice is not None and self.audio.get("voice_io_backend") in {
                "doubao_realtime",
                "qwen_modelscope_hybrid",
            }:
                ok = bool(self.realtime_voice.speak_text(text))
                if (
                    not ok
                    and self.audio.get("voice_io_backend") == "doubao_realtime"
                    and self.audio.get("tts_fallback_backend", "wake_skill_agent") == "wake_skill_agent"
                ):
    ...
    def _voice_args(self, seconds: float | None = None) -> argparse.Namespace:
        ...
        return argparse.Namespace(
            ...
            asr_wav_path=Path(self.audio.get("command_wav_path", "/home/test/qwen_robot_project/runtime/audio/command.wav")),
