...
def cmd_self_check(args: argparse.Namespace) -> int:
    ...
    voice_backend = str(config.get("audio", {}).get("voice_io_backend") or "")
    if voice_backend == "doubao_realtime":
    ...
    elif voice_backend == "qwen_modelscope_hybrid":
        qwen = config.get("voice_decision", {}).get("qwen", {})
        qwen = qwen if isinstance(qwen, dict) else {}
        env_path = Path(qwen.get("secret_env_file", "/home/test/.config/qwen_robot/modelscope.env"))
        env_text = env_path.read_text(encoding="utf-8") if env_path.exists() else ""
        add("modelscope_secret_file", env_path.exists(), str(env_path))
        add(
            "modelscope_api_key_configured",
            "MODELSCOPE_API_KEY=" in env_text or bool(os.environ.get("MODELSCOPE_API_KEY")),
            "environment or protected secret file",
        )
        zipformer_dir = Path(qwen.get("zipformer_model_dir", "/home/test/refine_0508/model"))
        for name in (
            ...
            add(f"qwen_local_asr_asset:{name}", (zipformer_dir / name).exists(), str(zipformer_dir / name))
        tts_archive = Path(
            qwen.get("local_tts_archive", "/home/test/refine_0508/model/tts_matcha_icefall_zh_en.tar.gz")
        )
        add("qwen_local_tts_archive", tts_archive.exists(), str(tts_archive))
...
def cmd_daemon(args: argparse.Namespace) -> int:
    ...
    voice_warmup = {"ok": True, "skipped": True, "reason": "realtime_voice_not_configured"}
    if getattr(orchestrator, "realtime_voice", None) is not None and bool(config.get("voice_decision", {}).get("warmup_on_start", False)):
        voice_warmup = orchestrator.realtime_voice.warmup()
    elif getattr(orchestrator, "realtime_voice", None) is not None:
        voice_warmup = {
            "ok": True,
            "backend": str(config.get("audio", {}).get("voice_io_backend") or "realtime_voice"),
            "skipped": True,
    ...
    reminder_runtime_dir = reminder_config.get("runtime_dir", "/home/test/qwen_single_function/runtime/agent_0625")
    reminder_store = ReminderStore(reminder_runtime_dir)
    reminder_audio = AudioManager(config, orchestrator.resources, realtime_voice=orchestrator.realtime_voice)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    daemon.add_argument("--post-wake-listen-delay-seconds", type=float, default=0.03)
    daemon.add_argument("--post-speech-listen-delay-seconds", type=float, default=0.10)
    daemon.add_argument("--followup-post-speech-listen-delay-seconds", type=float, default=0.10)
