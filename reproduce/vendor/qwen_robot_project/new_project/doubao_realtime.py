...
你是家用机器人 qwen_robot_project 的实时语音决策器。你只负责把用户语音转换成结构化 JSON；你不能直接调用 shell、不能直接执行 /home/test/qwen_single_function。
...
Compact decision wire format v1:
{
  ...
  "ask": null,
  "intent": {
    "speech_act": "explicit_command|implicit_request|question|hypothetical|conversation|social|unclear",
    "actionable": false,
...
- Negation, refusal, quoted/example speech, speculation, hypothetical wording,
  and diagnostic questions do not authorize a device action. Set
  speech_act=hypothetical for conditions the runtime cannot evaluate, set
  actionable=false, authorization=none, uncertain=true, and emit no tasks. A
  positive request to turn something off/close/stop is not negated.
- Questions that require current external data are executable retrieval tasks,
  not ordinary chat and not device mutations. Time, weather, traffic,
  location, and nearby-place questions use type=tasks,
  speech_act=question, actionable=true, authorization=explicit, and
  realtime_information. Preserve the whole utterance in args.query. Omit an
  unstated location so the configured home location is used. A clearly clipped
  leading syllable may be restored from context: after wake-up, “气怎么样” is
  the weather query “天气怎么样” and must create that task.
...
You must not execute shell commands and must not call /home/test/qwen_single_function
...
class DoubaoRealtimeSession:
    ...
    def _expand_compact_command_decision(self, parsed: dict[str, Any]) -> dict[str, Any]:
            ...
            group["steps"].append(
                {
                    ...
                    "reason": str(task.get("reason") or "流式语音语义决策"),
                }
            )

        task_groups = sorted(grouped.values(), key=lambda item: int(item.pop("_first_index", 0)))
        # ask is mutually exclusive with executable tasks.  Some providers
        # populate every advertised tool property even when the prompt says to
        # omit it; accepting such an acknowledgement as a real follow-up would
        # execute the task and then incorrectly ask the user another question.
        raw_ask = parsed.get("ask") if decision_type == "ask_user" else None
        ...
        if decision_type == "task_plan" and any(group.get("steps") for group in task_groups):
            expanded["reply"] = ""
            expanded["ask_user"] = None
    ...
    def _expand_compact_intent(raw_intent: Any, user_text: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "reason": raw_intent.get("reason") or "流式语音语义分析",
            "confidence": confidence,
        }
                ...
                steps.append(
                    {
                        ...
                        "reason": step.get("reason") or "流式语音语义决策",
                    }
                )
            copied = dict(group)
            copied["steps"] = steps
    ...
    def _known_navigation_points(self) -> list[dict[str, Any]]:
        candidates = [
            ...
            "/home/test/qwen_single_function/points/named_points.json",
        ]
