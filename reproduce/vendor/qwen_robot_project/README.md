...
这个隔离目录把唤醒、麦克风、喇叭、Qwen 语义决策、skill 调用、任务队列、中断栈和 history 放到同一套运行框架里。原 `/home/test/new_project` 与 `/home/test/single_function` 不参与写入。
...
3. 每次唤醒后的语音输入都是一个新的 `CommandSession`。本地 VAD 立即录音，常驻 RKNN Zipformer 直接识别，避免每轮约 11 秒的模型冷启动。
4. 识别文本交给 ModelScope 上的 `Qwen/Qwen3.5-35B-A3B`。Qwen 通过原生 Function Calling 流式提交紧凑语义决策；工具参数一旦成为完整 JSON 就立即返回执行器。ModelScope 当前没有为此密钥提供 Qwen Omni/Audio 托管 provider，因此本项目没有伪装成不存在的端到端 WebSocket 服务。
...
- 喇叭：`plughw:rockchiptas6424`，`aplay`；唤醒应答优先播放 `/home/test/self_program/runtime/wake_reply.pcm`，普通追问和结果播报使用常驻本地 Matcha TTS，不消耗豆包额度。
...
- 底盘运动：`/home/test/qwen_single_function/move_*`，底层 ROS2 `/cmd_vel`。
- 头部：`/home/test/qwen_single_function/head_control`，底层 `/step_motor_angle`。
- 导航：`/home/test/qwen_single_function/navigation_goto`，Nav2 `/navigate_to_pose`。
- 投影：`/home/test/qwen_single_function/projector_control`，I2C bus 2。
- 人脸、追踪、运动计数、环境感知和提醒都通过 `/home/test/qwen_single_function/<skill>/run.sh` 调用。
- `light_control` 和 `feeder_control` 通过米家云调用 `/home/test/qwen_single_function` 中的独立实现；`fan_control` 仍按 disabled skill 处理。

## 播报协议

`qwen_single_function` 不直接占用喇叭。每个 skill 通过 stdout 输出原本应该播报的文本；`/home/test/qwen_robot_project` 统一负责过滤、本地 TTS 和播放。这样长任务可以边执行边播报，例如运动计数中的“第一个”“第二个”和结束总结。

协议文件在：

```bash
/home/test/qwen_single_function/SPEECH_PROTOCOL.md
/home/test/qwen_single_function/speech_policy.json
```

每个 `/home/test/qwen_single_function/<skill>/skill.json` 也补充了 `speech_contract`，说明该 skill 的播报来源和事件类型。所有 `run.sh` 已加上 `PYTHONUNBUFFERED=1`，确保长任务进度可以实时被 `/home/test/qwen_robot_project` 读到。

## 命令

```bash
cd /home/test/qwen_robot_project
...
`config/hardware.json` 中的 `voice_decision.compact_decision_json=true` 会让 Qwen 通过
Function Calling 返回紧凑的语义决策。运行时再把 `tasks/ask/intent` 扩展成原有完整 TaskGroup，因此执行器、
多任务、追问、记忆、暗示性意图和会议投影后处理保持原有接口。解析器同时兼容旧版完整
JSON；把该开关设为 `false` 即可立即恢复旧提示格式。

流式接收会持续累计 Function Calling 参数，在参数第一次成为完整 JSON 时立即结束模型阶段。若紧凑任务缺少
...
每轮 Qwen 语音决策都会生成一个 `voice_<id>` trace，并把细粒度计时写入
`runtime/events/events.jsonl`。日志包含连接、等待人声、VAD、音频尾帧、ASR、
Qwen 首事件、工具名、工具参数首次可解析、TaskGroup 编排、技能进程、设备返回
