...
: config_(std::move(config))
{
  ...
  // Negative preserves the legacy behavior by inheriting the attempt TTL.
  // Zero means that a confirmed region lives until the exploration state is reset.
  if (config_.region_timeout_s < 0.0) {
    config_.region_timeout_s = config_.timeout_s;
  }
  config_.no_progress_timeout_s = std::max(0.1, config_.no_progress_timeout_s);
  config_.initial_no_progress_grace_s = std::max(0.0, config_.initial_no_progress_grace_s);
  config_.progress_epsilon_m = std::max(0.0, config_.progress_epsilon_m);
  config_.replan_distance_jump_threshold_m = std::max(
    0.0, config_.replan_distance_jump_threshold_m);
  config_.max_attempt_records = std::max(1, config_.max_attempt_records);
  config_.max_regions = std::max(1, config_.max_regions);
  config_.initial_feedback_samples_to_ignore = std::max(
    0, config_.initial_feedback_samples_to_ignore);
...
{
  // Failed-attempt buckets remain short-lived so isolated transient failures do not accumulate.
  ...
  // Confirmed regions may be session-long (zero) for static map defects such as
  // light leaks. They are still cleared by the normal exploration-session reset.
  if (config_.region_timeout_s > 0.0) {
    const int64_t region_ttl_ns = static_cast<int64_t>(config_.region_timeout_s * 1e9);
    for (auto it = state_.regions.begin(); it != state_.regions.end();) {
      if (now_ns - it->last_updated_ns >= region_ttl_ns) {
...
bool FrontierSuppression::all_frontiers_in_session_regions(
  const FrontierSequence & frontiers) const
{
  if (config_.region_timeout_s != 0.0 || frontiers.empty() || state_.regions.empty()) {
    return false;
  }
  return std::all_of(
    frontiers.begin(), frontiers.end(),
    [this](const FrontierLike & frontier) {
      return point_in_any_region(frontier_position(frontier));
    });
}
...
void FrontierSuppression::note_goal_progress(int dispatch_id, double distance_remaining, int64_t now_ns)
{
  ...
  ++progress.feedback_samples_seen;
  if (progress.feedback_samples_seen <= config_.initial_feedback_samples_to_ignore) {
    // Nav2 may publish distance_remaining before the global planner has produced
    // its stable full path. Never let those provisional values become the
    // historical minimum used by the no-progress watchdog.
    return;
  }

  if (!progress.has_feedback) {
    // The first sample after the ignored prefix is the stable baseline. Start
    // the no-progress window here rather than at goal dispatch time.
    progress.has_feedback = true;
    progress.initial_distance_confirmed = true;
...
bool FrontierSuppression::mark_timeout_cancel_if_needed(int dispatch_id, int64_t now_ns)
{
  ...
  const int64_t initial_grace_ns =
    static_cast<int64_t>(config_.initial_no_progress_grace_s * 1e9);
  if (now_ns - progress.started_ns < initial_grace_ns) {
    return false;
  }
