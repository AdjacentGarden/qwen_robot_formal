...
: Node("frontier_explorer", options)
{
  ...
  this->declare_parameter<double>("completion_confirmation_duration_s", 10.0);
  this->declare_parameter<int>("completion_confirmation_required_cycles", 3);
  ...
  this->declare_parameter<double>("frontier_suppression_region_timeout_s", 0.0);
  this->declare_parameter<int>("frontier_suppression_initial_feedback_samples_to_ignore", 3);
  this->declare_parameter<double>("frontier_suppression_no_progress_timeout_s", 20.0);
  this->declare_parameter<double>("frontier_suppression_initial_no_progress_grace_s", 0.0);
  this->declare_parameter<double>("frontier_suppression_progress_epsilon_m", 0.05);
  this->declare_parameter<double>(
    "frontier_suppression_replan_distance_jump_threshold_m", 1.0);
  ...
  params_.completion_confirmation_duration_s = std::max(
    0.0, this->get_parameter("completion_confirmation_duration_s").as_double());
  params_.completion_confirmation_required_cycles = std::max(
    1, static_cast<int>(this->get_parameter("completion_confirmation_required_cycles").as_int()));
  ...
  params_.frontier_suppression_region_timeout_s = this->get_parameter(
    "frontier_suppression_region_timeout_s").as_double();
  params_.frontier_suppression_initial_feedback_samples_to_ignore = this->get_parameter(
    "frontier_suppression_initial_feedback_samples_to_ignore").as_int();
  ...
  params_.frontier_suppression_initial_no_progress_grace_s = this->get_parameter(
    "frontier_suppression_initial_no_progress_grace_s").as_double();
  params_.frontier_suppression_progress_epsilon_m = this->get_parameter(
    "frontier_suppression_progress_epsilon_m").as_double();
  params_.frontier_suppression_replan_distance_jump_threshold_m = this->get_parameter(
    "frontier_suppression_replan_distance_jump_threshold_m").as_double();
  ...
  if (params_.frontier_suppression_enabled) {
    RCLCPP_INFO(
      this->get_logger(),
      "Frontier suppression enabled: threshold=%d, base=%.2fm, expansion=%.2fm, attempt_timeout=%.2fs, region_timeout=%.2fs (0=session), ignored_initial_feedback=%d, no_progress_timeout=%.2fs, startup_grace=%.2fs, max_attempts=%d, max_regions=%d",
      ...
      params_.frontier_suppression_region_timeout_s,
      params_.frontier_suppression_initial_feedback_samples_to_ignore,
