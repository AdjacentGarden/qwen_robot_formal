...
: params(std::move(params_in)), callbacks(std::move(callbacks_in))
{
  ...
  params.frontier_suppression_region_timeout_s = std::max(
    0.0,
    params.frontier_suppression_region_timeout_s);
  params.frontier_suppression_initial_feedback_samples_to_ignore = std::max(
    0,
    params.frontier_suppression_initial_feedback_samples_to_ignore);
  params.frontier_suppression_no_progress_timeout_s = std::max(
    0.1,
    params.frontier_suppression_no_progress_timeout_s);
  params.frontier_suppression_initial_no_progress_grace_s = std::max(
    0.0,
    params.frontier_suppression_initial_no_progress_grace_s);
  params.frontier_suppression_progress_epsilon_m = std::max(
    0.0,
    params.frontier_suppression_progress_epsilon_m);
  params.frontier_suppression_replan_distance_jump_threshold_m = std::max(
    0.0,
    params.frontier_suppression_replan_distance_jump_threshold_m);
