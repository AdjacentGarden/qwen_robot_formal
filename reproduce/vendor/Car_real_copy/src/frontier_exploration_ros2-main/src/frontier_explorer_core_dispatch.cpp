...
void FrontierExplorerCore::try_send_next_goal()
{
  ...
  const auto confirm_completion = [this, &current_pose](const std::string & reason) {
      const int64_t now_ns = callbacks.now_ns();
      if (!no_frontiers_started_at_ns.has_value()) {
        no_frontiers_started_at_ns = now_ns;
        consecutive_no_frontier_cycles = 1;
        callbacks.log_info(
          reason + "; starting completion confirmation; goal_in_progress=" +
          std::string(goal_in_progress ? "true" : "false") +
          ", last_frontier_result={" + last_frontier_result_summary + "}");
        return false;
      }

      ++consecutive_no_frontier_cycles;
      const double empty_duration_s = static_cast<double>(
        now_ns - *no_frontiers_started_at_ns) / 1e9;
      if (
        empty_duration_s < params.completion_confirmation_duration_s ||
        consecutive_no_frontier_cycles < params.completion_confirmation_required_cycles)
      {
        callbacks.log_debug(
          "Completion confirmation pending: empty_duration=" +
          std::to_string(empty_duration_s) + "s, empty_cycles=" +
          std::to_string(consecutive_no_frontier_cycles));
        return false;
      }

      if (!no_frontiers_reported) {
        callbacks.log_info(
          reason + " after confirmation: empty_duration=" +
          std::to_string(empty_duration_s) + "s, empty_cycles=" +
          std::to_string(consecutive_no_frontier_cycles));
  ...
  if (filtered_frontiers.empty() && !frontiers.empty()) {
    const bool permanently_suppressed = all_frontiers_permanently_suppressed(frontiers);
    ...
    if (!all_frontiers_suppressed_reported) {
      callbacks.log_info(
        ...
        "All currently detected frontiers are permanently suppressed for this exploration round" :
        "All currently detected frontiers are temporarily suppressed");
      all_frontiers_suppressed_reported = true;
    }
    handle_all_frontiers_suppressed(*current_pose);
    if (permanently_suppressed) {
      confirm_completion("No usable frontiers remain; all detected frontiers are permanently suppressed");
    } else {
      no_frontiers_started_at_ns.reset();
      consecutive_no_frontier_cycles = 0;
  ...
  if (filtered_frontiers.empty() && frontiers.empty() && params.escape_enabled && min_goal_distance > 0.0) {
    ...
    if (escape_filtered_frontiers.empty() && !escape_snapshot.frontiers.empty()) {
      const bool permanently_suppressed =
        all_frontiers_permanently_suppressed(escape_snapshot.frontiers);
      no_reachable_frontier_reported = false;
      publish_frontier_markers(escape_filtered_frontiers);
      if (!all_frontiers_suppressed_reported) {
        callbacks.log_info(
          permanently_suppressed ?
          "All currently detected escape-mode frontiers are permanently suppressed for this exploration round" :
          "All currently detected frontiers are temporarily suppressed");
        all_frontiers_suppressed_reported = true;
      }
      handle_all_frontiers_suppressed(*current_pose);
      if (permanently_suppressed) {
        confirm_completion(
          "No usable frontiers remain; all escape-mode frontiers are permanently suppressed");
      } else {
        no_frontiers_started_at_ns.reset();
        consecutive_no_frontier_cycles = 0;
        no_frontiers_reported = false;
  ...
  if (filtered_frontiers.empty()) {
    no_reachable_frontier_reported = false;
    publish_frontier_markers(filtered_frontiers);
    confirm_completion("No frontiers found");
    return;
  }
...
void FrontierExplorerCore::reset_exploration_runtime_state(bool clear_maps)
{
  ...
  no_frontiers_started_at_ns.reset();
  consecutive_no_frontier_cycles = 0;
  last_goal_feedback_info_log_time_ns.reset();
  last_frontier_result_summary = "none";
  return_to_start_started = false;
  return_to_start_completed = false;
  suppressed_return_to_start_started = false;
  cancel_request_in_progress = false;
  active_frontier_cancel_counts_as_failure = false;
...
void FrontierExplorerCore::request_active_goal_cancel(
  const std::string & reason,
  bool count_as_frontier_failure)
{
  if (active_goal_kind == "frontier" && count_as_frontier_failure) {
    // Multiple map/progress callbacks may request cancellation before Nav2
    // returns. Never let a later benign request erase failure attribution.
    active_frontier_cancel_counts_as_failure = true;
  }
...
void FrontierExplorerCore::issue_active_goal_cancel()
{
  ...
  callbacks.log_warn(
    "NAV_GOAL_CANCEL_REQUEST dispatch_id=" + std::to_string(current_dispatch_id) +
    ", kind=" + active_goal_kind + ", reason='" + reason + "'");
  ...
  bool bypass_min_distance_dispatch)
{
  ...
  for (; dispatch_index < frontier_sequence.size(); ++dispatch_index) {
    ...
    // A pre-dispatch block is a real failed attempt too. Remember it so the
    // suppression policy can stop the scheduler from selecting the same
    // costmap-blocked frontier on every map-processing tick.
    record_failed_frontier_attempt(
  ...
  const std::string & description)
{
  ...
  callbacks.log_info(description);
  {
    ...
    lifecycle_log << std::fixed << std::setprecision(2)
                  << "NAV_GOAL_SENT dispatch_id=" << dispatch_id
                  << ", kind=" << goal_kind
                  << ", frame=" << goal_pose.header.frame_id
                  << ", target=(" << goal_pose.pose.position.x
                  << ", " << goal_pose.pose.position.y << ")"
                  << ", queued_frontiers=" << frontier_sequence.size()
                  << ", goal_state=SENDING";
    callbacks.log_info(lifecycle_log.str());
  }
  ...
  const std::string & error_message)
{
  ...
  if (!accepted) {
    ...
    if (context.has_value() && context->goal_kind == "frontier") {
      ...
      last_frontier_result_summary =
        "dispatch_id=" + std::to_string(dispatch_id) +
  ...
  if (context.has_value() && context->goal_kind == "return_to_start") {
    ...
    std::ostringstream accepted_log;
    accepted_log << std::fixed << std::setprecision(2)
                 << "Frontier goal accepted: dispatch_id=" << dispatch_id;
    if (active_goal_pose.has_value()) {
      accepted_log << ", target=(" << active_goal_pose->pose.position.x
                   << ", " << active_goal_pose->pose.position.y << ")";
    }
    accepted_log << ", goal_in_progress=true, goal_state=ACTIVE; "
                 << "completion scheduling is blocked until Nav2 returns a terminal result";
    callbacks.log_info(accepted_log.str());
  }
  ...
  const std::string & exception_text)
{
  ...
  if (dispatch_id == current_dispatch_id) {
    const int64_t now_ns = callbacks.now_ns();
    const double elapsed_s = active_goal_sent_time_ns.has_value() ?
      static_cast<double>(now_ns - *active_goal_sent_time_ns) / 1e9 : -1.0;
    std::ostringstream result_log;
    result_log << std::fixed << std::setprecision(3)
               << "NAV_GOAL_RESULT dispatch_id=" << dispatch_id
               << ", kind=" << goal_kind
               << ", status=" << (exception_text.empty() ?
      detail::status_to_string(status) : "EXCEPTION")
               << ", error_code=" << error_code
               << ", elapsed=" << elapsed_s << "s";
    if (active_goal_pose.has_value()) {
      result_log << ", target=(" << active_goal_pose->pose.position.x
                 << ", " << active_goal_pose->pose.position.y << ")";
      const auto current_pose = callbacks.get_current_pose();
      if (current_pose.has_value()) {
        const double distance = std::hypot(
          active_goal_pose->pose.position.x - current_pose->position.x,
          active_goal_pose->pose.position.y - current_pose->position.y);
        result_log << ", robot=(" << current_pose->position.x
                   << ", " << current_pose->position.y << ")"
                   << ", distance_to_target=" << distance << "m";
      } else {
        result_log << ", distance_to_target=unknown(TF unavailable)";
      }
    }
    if (pending_cancel_reason.has_value()) {
      result_log << ", cancel_reason='" << *pending_cancel_reason << "'";
    }
    if (!error_message.empty()) {
      result_log << ", error_msg='" << error_message << "'";
    }
    if (!exception_text.empty()) {
      result_log << ", exception='" << exception_text << "'";
    }
    callbacks.log_info(result_log.str());
    if (goal_kind == "frontier") {
      last_frontier_result_summary = result_log.str();
    }
  }
  ...
  if (exception_text.empty()) {
      ...
      goal_kind == "suppressed_return_to_start")
    {
      ...
      callbacks.log_warn(goal_kind + " was canceled before completion");
      if (
        goal_kind == "frontier" &&
        active_frontier_cancel_counts_as_failure)
      {
...
void FrontierExplorerCore::feedback_callback(double distance_remaining, int dispatch_id)
{
  ...
  const int64_t now_ns = callbacks.now_ns();
  constexpr int64_t kFeedbackInfoPeriodNs = 2'000'000'000LL;
  if (!last_goal_feedback_info_log_time_ns.has_value() ||
    now_ns - *last_goal_feedback_info_log_time_ns >= kFeedbackInfoPeriodNs)
  {
    last_goal_feedback_info_log_time_ns = now_ns;
    std::ostringstream info;
    info << std::fixed << std::setprecision(3)
         << "NAV_GOAL_FEEDBACK dispatch_id=" << dispatch_id
         << ", kind=" << active_goal_kind
         << ", distance_remaining=" << distance_remaining << "m";
    if (active_goal_pose.has_value()) {
      info << ", target=(" << active_goal_pose->pose.position.x
           << ", " << active_goal_pose->pose.position.y << ")";
    }
    callbacks.log_info(info.str());
  }
