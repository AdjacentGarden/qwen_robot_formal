...
struct FrontierSuppressionConfig
{
  ...
  // TTL for transient attempt records.
  ...
  // Initial grace window after goal acceptance before no-progress cancelation can fire.
  double initial_no_progress_grace_s{0.0};
  // If the second feedback jumps upward by this amount, discard the first
  // feedback as a transient partial-path estimate. This is intentionally only
  // applied during initial baseline confirmation so later replans cannot keep
  // resetting the no-progress watchdog.
  double replan_distance_jump_threshold_m{1.0};
  // TTL for confirmed suppressed regions. Zero keeps regions for the entire
  // exploration session; a positive value expires them after that many seconds.
  double region_timeout_s{-1.0};
  // Number of early Nav2 feedback samples ignored before accepting a stable
  // distance_remaining baseline. Nav2 may initially report a partial path.
  int initial_feedback_samples_to_ignore{3};
};
...
struct ActiveGoalProgress
{
  ...
  bool initial_distance_confirmed{false};
...
public:
  ...
  // True only when every supplied goal lies inside a confirmed region whose
  // lifetime is the complete exploration session. Attempt records do not count.
  bool all_frontiers_in_session_regions(const FrontierSequence & frontiers) const;
