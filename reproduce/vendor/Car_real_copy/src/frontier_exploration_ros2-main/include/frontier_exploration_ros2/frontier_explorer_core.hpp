...
struct FrontierExplorerCoreParams
{
  ...
  double completion_confirmation_duration_s{10.0};
  int completion_confirmation_required_cycles{3};
  ...
  double frontier_suppression_region_timeout_s{-1.0};
  int frontier_suppression_initial_feedback_samples_to_ignore{3};
  double frontier_suppression_no_progress_timeout_s{20.0};
  double frontier_suppression_initial_no_progress_grace_s{0.0};
  double frontier_suppression_progress_epsilon_m{0.05};
  double frontier_suppression_replan_distance_jump_threshold_m{1.0};
...
public:
  ...
  void request_active_goal_cancel(
    const std::string & reason,
    bool count_as_frontier_failure = false);
  ...
  std::optional<int64_t> last_goal_feedback_info_log_time_ns;
  std::string last_frontier_result_summary{"none"};
  ...
  std::optional<int64_t> no_frontiers_started_at_ns;
  int consecutive_no_frontier_cycles{0};
  ...
  // Sticky for the current dispatch until its terminal result arrives. This is
  // independent of the human-readable cancel reason, which is consumed when
  // the asynchronous cancel request is issued.
  bool active_frontier_cancel_counts_as_failure{false};
...
private:
  ...
  bool all_frontiers_permanently_suppressed(const FrontierSequence & frontiers) const;
