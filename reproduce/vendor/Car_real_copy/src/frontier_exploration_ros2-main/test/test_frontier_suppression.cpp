...
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    2, 2.0, 1.0, 5.0, 20.0, 0.25, 8, 4, 0.3, 0.0, 1.0, 5.0});
...
TEST(FrontierSuppressionTests, SessionRegionDoesNotExpire)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    2, 2.0, 1.0, 5.0, 20.0, 0.25, 8, 4, 0.3, 0.0, 1.0, 0.0});

  suppression.record_failed_attempt(make_candidate(3.0, 3.0), 1'000'000'000);
  suppression.record_failed_attempt(make_candidate(3.0, 3.0), 2'000'000'000);
  ASSERT_EQ(suppression.region_count(), 1U);

  suppression.prune_expired(3'602'000'000'000);
  EXPECT_EQ(suppression.region_count(), 1U);
}

TEST(FrontierSuppressionTests, PermanentCoverageRequiresConfirmedSessionRegion)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    2, 2.0, 1.0, 90.0, 20.0, 0.25, 8, 4, 0.3, 0.0, 1.0, 0.0});
  const FrontierSequence frontiers{make_candidate(2.0, 2.0)};

  suppression.record_failed_attempt(frontiers.front(), 1'000'000'000);
  EXPECT_FALSE(suppression.all_frontiers_in_session_regions(frontiers));
  EXPECT_EQ(suppression.region_count(), 0U);

  suppression.record_failed_attempt(frontiers.front(), 2'000'000'000);
  EXPECT_TRUE(suppression.all_frontiers_in_session_regions(frontiers));
  EXPECT_EQ(suppression.region_count(), 1U);

  const FrontierSequence partly_outside{
    make_candidate(2.0, 2.0), make_candidate(8.0, 8.0)};
  EXPECT_FALSE(suppression.all_frontiers_in_session_regions(partly_outside));
}
...
TEST(FrontierSuppressionTests, MeaningfulProgressResetsTimeout)
{
  ...
  suppression.note_goal_progress(7, 1.0, 500'000'000);
  suppression.note_goal_progress(7, 2.0, 1'000'000'000);
  suppression.note_goal_progress(7, 3.0, 1'500'000'000);
  suppression.note_goal_progress(7, 10.0, 2'000'000'000);
...
TEST(FrontierSuppressionTests, FirstThreeFeedbackSamplesAreIgnored)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    3, 2.0, 1.0, 90.0, 5.0, 0.5, 8, 4, 0.3, 0.0, 1.0});

  suppression.start_goal_progress_tracking(8, 0);
  suppression.note_goal_progress(8, 0.9, 500'000'000);
  suppression.note_goal_progress(8, 1.0, 1'000'000'000);
  suppression.note_goal_progress(8, 1.1, 1'500'000'000);
  suppression.note_goal_progress(8, 15.0, 2'000'000'000);

  // The fourth sample establishes the baseline and starts the watchdog window.
  EXPECT_FALSE(suppression.mark_timeout_cancel_if_needed(8, 6'000'000'000));
  EXPECT_TRUE(suppression.mark_timeout_cancel_if_needed(8, 8'000'000'000));
}

TEST(FrontierSuppressionTests, ProgressAfterFourthSampleBaselineResetsTimeout)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    3, 2.0, 1.0, 90.0, 5.0, 0.5, 8, 4, 0.3, 0.0, 1.0});

  suppression.start_goal_progress_tracking(9, 0);
  suppression.note_goal_progress(9, 0.9, 500'000'000);
  suppression.note_goal_progress(9, 1.0, 1'000'000'000);
  suppression.note_goal_progress(9, 1.1, 1'500'000'000);
  suppression.note_goal_progress(9, 15.0, 2'000'000'000);
  suppression.note_goal_progress(9, 14.4, 3'000'000'000);

  EXPECT_FALSE(suppression.mark_timeout_cancel_if_needed(9, 7'000'000'000));
  EXPECT_TRUE(suppression.mark_timeout_cancel_if_needed(9, 9'000'000'000));
}

TEST(FrontierSuppressionTests, LaterUpwardReplansCannotExtendTimeout)
{
  FrontierSuppression suppression(FrontierSuppressionConfig{
    3, 2.0, 1.0, 90.0, 5.0, 0.5, 8, 4, 0.3, 0.0, 1.0});

  suppression.start_goal_progress_tracking(10, 0);
  suppression.note_goal_progress(10, 10.0, 0);
  suppression.note_goal_progress(10, 12.0, 1'000'000'000);
  suppression.note_goal_progress(10, 14.0, 3'000'000'000);

  EXPECT_TRUE(suppression.mark_timeout_cancel_if_needed(10, 6'000'000'000));
}
...
TEST(FrontierSuppressionCoreTests, PreDispatchBlockedFrontierBecomesSuppressed)
{
  int64_t now_ns = 1'000'000'000;
  int dispatch_calls = 0;
  std::vector<std::string> info_logs;
  ...
  auto local_costmap_msg = build_grid(20, 20, 0);
  local_costmap_msg.data[4 * 20 + 4] = 59;
  ...
  // The next scheduling cycle filters the same candidate before dispatch.
  core->try_send_next_goal();
  EXPECT_EQ(dispatch_calls, 0);
}
...
TEST(FrontierSuppressionCoreTests, ActiveGoalBlockedByCostmapCreatesSuppressedRegion)
{
  int64_t now_ns = 1'000'000'000;
  int dispatch_calls = 0;
  auto core = make_suppression_core(&now_ns, &dispatch_calls, nullptr, nullptr);
  ...
  blocked_costmap.data[4 * 20 + 4] = 99;
  core->costmapCallback(OccupancyGrid2d(blocked_costmap));
...
TEST(FrontierSuppressionCoreTests, AllPermanentlySuppressedCompletesCurrentRoundAfterConfirmation)
{
  int64_t now_ns = 1'000'000'000;
  int dispatch_calls = 0;
  ...
  params.frontier_suppression_attempt_threshold = 1;
  params.frontier_suppression_startup_grace_period_s = 0.0;
  params.frontier_suppression_region_timeout_s = 0.0;
  params.completion_confirmation_duration_s = 2.0;
  params.completion_confirmation_required_cycles = 3;
  params.return_to_start_on_complete = false;
  ...
  callbacks.on_exploration_complete = [&completion_calls]() {++completion_calls;};
  callbacks.log_info = [](const std::string &) {};
  callbacks.log_warn = [](const std::string &) {};
  callbacks.log_debug = [](const std::string &) {};
  callbacks.log_error = [](const std::string &) {};
  ...
  core.costmap = OccupancyGrid2d(build_grid(20, 20, 0));
  core.map_generation = 1;
  core.costmap_generation = 1;
