...
def _execute_action(
    ...
    if action == "navigation":
        ...
        if not args.rviz:
            print("warning: --no-rviz is ignored in fixed 3-terminal navigation mode")
        ok = flow.start_navigation(launch_args=None, force_restart=args.restart)
        if ok:
            if was_running and args.restart:
                ...
                print("launch order: real_robot_base -> real_robot_odometry -> real_robot_nav")
        else:
